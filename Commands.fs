...
module Cart = 
  ...
  type Client (conn: SqliteConnection) =
    ...
    member __.CreateCart cartId (state: CustomerCart) =
      ...
      then Error (CartAlreadyExists state)
      else write <| CartCreated cartId <| state

    member __.AddItemToCart cartId product (state: CustomerCart) =
      if state.Status <> CartStatus.Ready
      then Error (CartCannotBeChanged state)
      else write <| ItemAddedToCart (cartData cartId product) <| state

    member __.RemoveItemFromCart cartId product (state: CustomerCart) =
      if state.Status <> CartStatus.Ready
      then Error (CartCannotBeChanged state)
      else write <| ItemRemovedFromCart (cartData cartId product) <| state

    member __.CheckOut cartId (state: CustomerCart) =
      if state.Status <> CartStatus.Ready
      then Error (CartCannotBeCheckedOut state)
      else write <| CheckedOut cartId <| state

    member __.Shipped cartId (state: CustomerCart) =
      if state.Status <> CartStatus.CheckedOut
      then Error (CartCannotBeShipped state)
      else write <| ShippedOrder cartId <| state